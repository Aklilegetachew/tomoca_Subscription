...
function setmiscTwo($user_id)
{
    global $db;
    $query = "UPDATE users SET misc = 2 WHERE UserId=$user_id";;
    $res = mysqli_query($db, $query);
    return $res;
}
...
function PostToChannel($channelName, $product_title, $product_image, $product_price, $product_Desc, $productId, $data, $product_size, $Product_roast)
{
    ...
    $caption = urlencode("*Product:*" . $product_title . "\n\n" . "*Product type:* " . $product_Desc . "\n\n" . "*Roast type:* " . $Product_roast . "\n\n" . "*Size:* " . $product_size . "\n\n" . "*Price:* " . $product_price . "ETB" . "\n");

    $markup  = array('inline_keyboard' => array(array(array('text' => 'Buy now', 'url' => 'http://t.me/Tomoca_bot?start=' . $productId, 'callback_data' => 'Buynow',))));
...
function SendNotificationMsg($UserInfo, $msg)
{
    ...
    $detailText = urlencode("
    🔔" . $msg . "\n\nCustomer Name:" . $Fullname  . "\n\nPhone Number:" . $phoneNumber . "\n\n " . "Type of Product:" . $NumProduct . "\n\nTotal Amount:" . $TotalAmount . "\n\nShop Location:" . $shopLocation . "\n\nOrder Date:" . $dateOrder . "\n\nOrder Time:" . $timeOrder . "\n\n");

    message(5102867263, $detailText);
    message(5692054781, $detailText);
    message(2137520677, $detailText);
    message(379252630, $detailText);
...
function adduser($first_name, $Last_name, $user_id, $product_Id, $MSGID)
{
    ...
    if (empty($res2)) {
        $query = "INSERT INTO users(UserName, UserId, userProductid, LastName, StartID, phase) VALUES ('$first_name', '$user_id', '$product_Id', '$Last_name', '$MSGID', 'true')";
        $res = mysqli_query($db, $query);
        if (!$res) {
            die('query failed' . mysqli_error($db));
        }
