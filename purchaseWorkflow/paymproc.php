...
function orderType()
{
  ...
  // array(array('text' => 'Delivery Order')),
  $markup  = array('keyboard' => array(array(array('text' => 'Pickup Order')), array(array('text' => 'Back'), array('text' => 'Cancel Order'))), 'resize_keyboard' => true, 'one_time_keyboard' => true, 'selective' => true);
  $markupjs = json_encode($markup);
...
function DetailText($chat_id)
{
  $markup  = array('keyboard' => array(array('Next'), array('Go Back', 'Cancel')), 'resize_keyboard' => true, 'selective' => true, 'one_time_keyboard' => true);
  $markupjs = json_encode($markup);
  $ret = message($chat_id, "🛒 Cart Detail ዝርዝር ", $markupjs);
}
